...
#include <stdint.h>
#endif
EXTERN_C uint16_t getCommServicePort ( bool useLocalHost ) ;
#if defined(_MSC_VER)
EXTERN_C int addOutBoundRule ( size_t port , const char * program ) ;
#endif
...
EXTERN_C void TgtConnBackgroundTask ( ) { } EXTERN_C const char_T *
TgtConnInit ( int_T argc , char_T * argv [ ] ) { const char_T * result = (
NULL ) ; { int_T tvar ; for ( tvar = 1 ; tvar < argc ; tvar ++ ) { if ( argv
[ tvar ] == NULL ) continue ; if ( strcmp ( argv [ tvar ] , "-tgtconn_port" )
...
NULL ; continue ; } } port = getCommServicePort ( true ) ;
#if defined(_MSC_VER)
addOutBoundRule ( ( size_t ) port , argv [ 0 ] ) ;
#endif
if ( writePortNumToServerInfoFile ) { FILE * fh ; fh = fopen (
serverInfoFileName , "w" ) ; if ( fh == NULL ) { result =
