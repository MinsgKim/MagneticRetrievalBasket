...
Sim.Plate_MagnetMagnitude = ones(1,6)*160000*Sim.PlateVolume;
Sim.Stiffness = 2.4e-6;
Sim.Stiffness_upper = 4e-6;

Sim.BField_Magnitude = 10;
Sim.BField_tilt = 0;
...
Fitness_ = optim_bending_and_stability(Sim.Angle, Sim, "GUI", "off", "DispInfo", "no", "InputAngleUnit", "Radian");
...
% Sim.Plate_MagnetMagnitude = [31105 30247 39991 30001 31387 39941]*Sim.PlateVolume;
% Sim.Stiffness = 2.4e-6;
% Sim.Stiffness_upper = 4e-6;
...
% data
measured_data = [815 327; 875 326; 960 330; 1038 319; 1102 262;...
    1123 194; 1125 119];

measured_angle = zeros(1,7);
measured_q = zeros(1,6);

for i=1:6

    x(i) = measured_data(i,1)-measured_data(i+1,1);
    y(i) = measured_data(i,2)-measured_data(i+1,2);
    measured_angle(i+1) = rad2deg(atan(abs(y(i)./x(i))));
    measured_q(i) = measured_angle(i+1) - measured_angle(i);
...
% x_init = [[31000 31000 32000 31000 32000 35000]*Sim.PlateVolume, [2e-6 3e-6]* 1e3];
%
% lb = [ones(1,6)*30000*Sim.PlateVolume, [0.1e-6 0.1e-6]* 1e3];
% ub = [ones(1,6)*40000*Sim.PlateVolume, [3e-6 4e-6] * 1e3];
...
options = optimoptions('fmincon', 'Display', 'iter', 'StepTolerance', 1e-10, 'FunctionTolerance', 1e-10, ...
    'MaxFunctionEvaluations', 1e5, 'OptimalityTolerance', 1e-10, ...
...
if strcmpi(select_stiff_ver, 'remanence')
    ...
    Sim.Stiffness = 2.4e-6;
    Sim.Stiffness_upper = 4e-6;
    Sim.Damping = 5e-9*2; % [Nm/(radian/s)]
    Sim.Damping_upper = 5e-9*2;
...
x_init = deg2rad([25 30 45 -45 -35 -25]);
