...
x_init = Input.x_init_1;

% optimizing boundaries
lb = [repmat(-pi, 1, Fixed.num_links), repmat(5e3, 1, Fixed.num_links +1) * Fixed.PlateVolume];
ub = [repmat(pi, 1, Fixed.num_links), repmat(4e4, 1, Fixed.num_links +1) * Fixed.PlateVolume];
...
x_round(1:Fixed.num_links) = deg2rad([55 25 30 45 -45 -35 -20]);
x_round(Fixed.num_links+1:2*Fixed.num_links) = repmat(3.5e4,1,7)*Fixed.PlateVolume;

Simscape_Fitness(x_round, SimParams, Fixed, "DispInfo", "yes", "GUI", "on") 
