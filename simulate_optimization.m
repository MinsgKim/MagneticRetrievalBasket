...
% fundamental setup:
num_links = 7;          % # of robot links
link_length = 2e-3;     % 2 mm
cross_section_area = 0.0033 * 0.0005; % cross section area for magnetization (3.3 mm X 0.5 mm)
k_spring_rot = 6.88e-8 * ones(1, num_links-1); % rotational spring constants

r_ext = 0.025;           % y-directional distance from external magnet, 40 mm
EM = External_Magnet(); % external magnet object

%--------------------------------------------------
% optimization parameters: psi, thetaM
%--------------------------------------------------
% magnetization range (A/m)
lb_psi = 2e4; ub_psi = 6e4;
% magnetization direction range (rad)
lb_theta_M = -pi; ub_theta_M = pi;

% initial values
rng(0);
psi_init = 4e4*(rand(1,num_links)+0.5);     % from 20,000 to 60,000
theta_M_init = rand(1,num_links)*2*pi-pi;   % from -pi to pi
x0 = [psi_init, theta_M_init];
